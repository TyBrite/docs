...
Every store gets a full set of business books — a chart of accounts, double-entry journals, and the
standard financial statements — which Galactic Core keeps up to date as the store trades. A merchant does
not post entries by hand: a sale, its cost of goods, a refund, an expense, a gift card each write the
correct balanced entry the moment the business event happens.
...
If an entry cannot be written, the sale or refund itself still goes through: the books flag the gap rather
than blocking the transaction, and a background process reconciles anything outstanding.

## The reports a merchant gets

The standard financial statements come straight out of the entries — a **trial balance**, an **income
statement (profit & loss)**, a **balance sheet**, and a **cash-flow** view — alongside the underlying
**journals** and the **chart of accounts** itself, all in the store's admin. Because Galactic Core holds
the store's costs as well as every sale, the profit figures are computed from recorded cost of goods
