...
## The catalogue an operator's stores start with

An operator defines the categories every store they onboard is created with. They can start from the
standard set and cut it to what their market sells, add categories of their own, and relabel any of
them — the name a merchant reads is separate from the name the platform matches on, so relabelling
changes nothing underneath.

Alongside the categories, an operator sets three things that decide how finished a store looks on its
first day: a starter image per category, starter subcategories offered on top of the suggestions every
store already gets, and a product specification per category that replaces the standard one.

Their merchants keep exactly what any merchant has. A merchant switches a category on or off, sets its
image, and owns their subcategories outright — creating, renaming, nesting and removing them freely.
What no merchant does, on any deployment, is create, rename or delete a category.

Two boundaries are worth knowing before designing a catalogue around them.

**Names are permanent.** A category's name is what links a product to its specification template, its
starter subcategories and its storefront styling, so it is fixed once saved — for the operator as well
as the merchant. Relabelling is always available; renaming is not.

**Nothing is deleted.** Removing a category from the catalogue stops it reaching new stores; stores
already created keep it, along with the products, subcategories and templates filed under it. The same
applies to a category image, which can be replaced but not cleared, because a storefront laying out
category tiles has nothing to show in place of one.

A category of an operator's own works everywhere a standard one does. What it does not carry is the
ready-made styling a generated storefront applies to the standard set, so products in it fall back to
a generic look until the operator supplies their own subcategories and specification.
...
## A marketplace across an operator's merchants

An operator running a platform can also run a marketplace over the merchants already on it: one
storefront spanning their catalogues, one basket that crosses several of them, and a commission on
each sale. The merchants stay where they are — their own storefronts, orders and customers are
untouched, and a merchant sells under their own brand and through the marketplace from the same
store.

Such a marketplace is closed. Its sellers are that operator's own merchants and nobody else, so
there is no public application route by which an outside business joins. A merchant takes part by
default and can leave from their own settings.

Settlement follows the same rule as the rest of this page: shoppers pay onto the operator's own
payment account, each seller is paid their portion, and the commission stays with the operator.

A marketplace does not have to occupy a deployment of its own. Running on dedicated infrastructure
is a capacity and blast-radius choice — what it buys is headroom sized to one operator's traffic,
not the data boundary, which holds either way.

**Reference:** [Agencies](/agencies#running-a-marketplace-as-well) · [Marketplaces at Scale](/how-gc-works/marketplace-scale)
