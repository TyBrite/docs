...
- Orders, customers, cart items, and wishlist entries created with a `test` key are stored in the sandbox partition and are never returned to `live` key requests.
