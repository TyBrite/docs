...
<Tip>
  A unique `Idempotency-Key` on order and payment calls does two things: it guarantees
  exactly-once processing, and it makes those requests eligible for automatic retry.
</Tip>
...
List endpoints paginate by cursor rather than by offset, so a row inserted or removed between requests does not shift the page boundary and cause an item to repeat or be skipped.

A paginated query returns a `pagination` object alongside the results. Its `next_cursor` property fetches the following page:
...
- [Pricing](/sdk/api-reference/classes/PricingService)
- [Orders](/sdk/api-reference/classes/OrdersService)
- [Promotions](/sdk/api-reference/classes/PromotionsService)
- [Reviews](/sdk/api-reference/classes/ReviewsService)
- [CMS](/sdk/api-reference/classes/CmsService)
- [Messaging](/sdk/api-reference/classes/MessagingService)
- [Webhooks](/sdk/api-reference/classes/WebhooksService)
- [Messaging](/sdk/api-reference/classes/MessagingService)
- [Webhooks](/sdk/api-reference/classes/WebhooksService)

## Service Overview

Each service is accessible as a property on the `Tybrite` client. The most commonly used ones are below; the [API reference](/sdk/api-reference/classes/ProductsService) covers the full set, including returns, disputes, reviews, tax, B2B, and the ingestion and sandbox tooling. The "Customer auth?" column indicates whether the customer session token (`xAuthToken`) is required for at least some methods.
