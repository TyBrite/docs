...
Cart and wishlist entries are held against a specific variant rather than a product, so a saved selection carries its own colour and size, its own price, its current stock level, and the images belonging to that variant.
...
// returns { items: [...] } — the full updated cart (same shape as getCart)
```
...
// returns { items: [...] } — the full updated cart with the new quantity + recomputed total_price
```
...
// returns { items: [] } — the now-empty cart
```
...
// returns { items: [...] } — the cart with that item removed
```
...
// returns { items: [...] } — the merged customer cart (guest lines folded in)
```
...
// returns 201 { items: [...] } — the full wishlist including the newly saved item
...
// returns { items: [...] } — the wishlist with that item removed
```
...
const handleAddToCart = async (selectedVariant: Variant, quantity: number) => {
  try {
    if (customerSession?.access_token) {
      ...
      // returns { items: [...] } — re-render the cart badge from items.length
    } else {
