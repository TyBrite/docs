...
A store's categories are settled when the store is created and are not authored afterwards, so a
category slug keeps meaning the same thing for the life of that store — which makes it safe to
hard-code in a storefront route. Across stores, read `listCategories` for the store you are
integrating with rather than assuming a shared list. Subcategory slugs are unique within their
store, category and parent; where two names in that scope would reduce to the same slug, a numeric
