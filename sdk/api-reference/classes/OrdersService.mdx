...
const order = await client.orders.createOrder({
  ...
  requestBody: {
    ...
    payment_method: 'stripe', // stripe, paystack, mpesa, airtel_money, cash
    shipping_metadata: {
      fee: 200,
      applied_rule: 'zone',
      zone_name: 'Downtown'
  ...
  xSignature: 'computed-signature-here'
});
```
...
const order = await client.orders.getOrder({
  id: 'order-uuid-here',
  fields: 'order_number,total_amount,order_status,items' // Optional
});
```
