...
The `OrdersService` class (accessed via `client.orders`) covers the order lifecycle: reserving stock at checkout, creating an order, retrieving and listing orders, and moving one through its payment and fulfilment states. Creating and updating an order additionally carry an **HMAC signature** over the request body, so the server can confirm the payload was not altered in transit.
...
<Note>
  An order and each of its line items carry a `custom_fields` array — fields the merchant defined themselves, such as a buyer's purchase-order number. `createOrder` accepts values for them at checkout. See [Custom Fields](/custom-fields).
</Note>
...
The listing returns the order rows themselves. Line items, status notes and custom fields are fetched
per order and belong to `getOrder`; asking for one of them in `?fields=` here returns `400` naming
the field rather than omitting it silently.
...
Reservations are all-or-nothing: if any item lacks available stock, nothing is reserved and you get `409 insufficient_stock`. Each hold expires automatically (15 minutes by default), so an abandoned checkout lets the hold lapse without an explicit release call. Works with publishable or secret keys.
...
// returns { order: { id, order_number, payment_status: 'paid', total_amount, … }, post_processing_warnings? }
```
...
*   **`202 Accepted` under heavy load.** Some deployments defer order writes during a sustained order surge to keep the storefront responsive — the order is accepted and created a moment later rather than synchronously. On those deployments, and only while under that load, `createOrder` returns **`202`** with `{ status: 'accepted', idempotency_key, message }` and **no order in the body**. Treat it as success-in-progress: fetch the order once it exists by replaying `createOrder` with the same `Idempotency-Key` (which returns `200` with the order), or by polling `getOrder` if you have the id. Below that load — and on deployments that do not defer writes — creation is synchronous and returns `201` as above. A client that cannot handle a `202` (does not poll) should only be used against deployments where deferral is off.
...
*   **`currency`** — the ISO 4217 code the order is priced in. Defaults to the store's default storefront currency. Set it when you present prices in a currency the shopper picked, so the order is recorded in the currency they actually paid in.
*   **`order_number`** — your own order reference. Galactic Core generates one when you omit it; pass your own to keep numbering aligned with a system you already run.
*   **Attribution (optional)**: pass `attribution` to record where the sale came from, so it is credited to the right traffic source in the merchant's reports: `{ session_id, utm_source, utm_medium, utm_campaign, utm_term, utm_content, referrer }`, plus `content_id` + `content_type` to close the loop on a shopper who arrived from a blog post or lookbook. `utm_term` and `utm_content` carry the paid keyword and the creative, so a merchant can compare one ad variant against another rather than only one source against another. The same fields are also accepted at the top level of the body if you already send them that way. Promotions are recorded from the server-validated result rather than what you pass, so the record always reflects the discount the shopper actually received.
...
```json Response
{
  "order": {
    ...
    "customer_email": "jane.doe@tybritelabs.com",
    "customer_name": "Jane Doe",
    "subtotal": 598,
    "tax_amount": 53.08,
...
The response carries `status_notes` — customer-visible updates the merchant added as the order
moved. These cover the stretch between payment and dispatch (preparation, checks, packing) that a
shopper otherwise hears nothing about, so an order-status page reads as a timeline rather than a
single status word. Oldest first. Internal notes are never included.

```typescript
for (const note of order.status_notes ?? []) {
  console.log(note.created_at, note.to_status, note.body);
}
```
...
| `422` | `POST /v1/orders` | `order_rejected` — the store runs its own order-validation rule and that rule declined the order; the message carries the store's reason. No order is created. Also returned when the store requires validation but the rule could not be reached, so an order is never created unchecked. |
| `422` | `PATCH /v1/orders/{id}` | `payment_not_verified` — you marked the order paid with a `payment_reference`, but the payment method the merchant added does not report that reference as paid. The order is left unchanged rather than accepted on an unverified claim. |
