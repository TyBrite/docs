...
*   **Attribution (optional)**: pass `attribution` to record where the sale came from, so it is credited to the right traffic source in the merchant's reports: `{ session_id, utm_source, utm_medium, utm_campaign, referrer }`, plus `content_id` + `content_type` to close the loop on a shopper who arrived from a blog post or lookbook. The same fields are also accepted at the top level of the body if you already send them that way. Promotions are recorded from the server-validated result rather than what you pass, so the record always reflects the discount the shopper actually received.
