...
The `RecommendationsService` class (accessed via `client.recommendations`) returns product suggestions for a storefront — products similar to one a shopper is viewing, products frequently bought together, and the item a shopper is most likely to look at next.
...
// Suggestions tailored to the signed-in shopper's preferences and history.
// Identify the shopper with a credential header (x-auth-token, x-external-auth, or x-idp-token) —
// personalization is keyed to whoever that credential resolves to, not to a customer id in the body.
const { recommendations } = await client.recommendations.getRecommendations({
  requestBody: {
    type: 'personalized',
    limit: 6
  }
  // pass the shopper's credential header on the request, e.g. { headers: { 'x-auth-token': token } }
});
// recommendations[0] => { productId: '0186665f-f0a1-4c79-976a-919b5b1c2daa',
//                         score: 1, reason: 'Trending #1 - 2.1 units/day' }
...
| `personalized` | Suggestions tailored to the signed-in shopper's history. | a shopper credential header (`x-auth-token` / `x-external-auth` / `x-idp-token`) |
...
For `personalized`, identify the shopper with one of the credential headers `x-auth-token`,
`x-external-auth`, or `x-idp-token` on the request. Personalization is keyed to whoever that credential
resolves to; with no valid credential the response falls back to trending.
...
| `400` | Invalid `type` enum or missing required field (`productId` for `similar`/`also-bought`/`bundle`). |
