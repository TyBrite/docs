...
const { methods } = await client.payments.getPaymentMethods({
  fields: 'provider,display_name,supported_currencies' // Optional
});

methods.forEach(method => {
  console.log(`Provider: ${method.provider} | Currencies: ${method.supported_currencies.join(', ')}`);
});
```
...
const response = await client.payments.initializePayment({
  requestBody: {
    ...
    idempotency_key: 'unique-session-id' // Recommended
  }
});
