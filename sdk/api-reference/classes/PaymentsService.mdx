...
Retrieve all active payment methods configured in GC Admin. This endpoint accepts **both publishable (`pk_*`) and secret (`sk_*`) API keys** — storefronts can safely call it client-side to discover available providers before checkout.
...
const paymentData = {
  ...
  amount: 5000,
  currency: 'USD',
  email: 'john.doe@tybritelabs.com',
...
| `409` | `provider_not_live` — the payment method is still configured with test credentials, so it cannot take a real payment. The merchant switches it to live mode in their dashboard. Sandbox requests are unaffected. |
...
| `502` | `provider_unavailable` — a payment method the merchant added themselves did not complete the request. Retry, or offer another method. |
| `504` | `provider_timeout` — the provider did not respond in time. Safe to retry. |
...
| `404` | No payment matches the `reference`. |
| `429` | Rate limit exceeded. |
| `500` | Server error. |
| `502` | `provider_unavailable` — a payment method the merchant added themselves did not complete the verification. The payment's real state is unchanged; retry. |
| `504` | `provider_timeout` — the provider did not respond in time. Safe to retry. |
