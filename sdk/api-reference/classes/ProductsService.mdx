...
#### Wholesale pricing for a signed-in buyer

Pass `b2b: true` together with a wholesale buyer's session token and each product is priced against
that buyer's own agreement rather than at the shelf price, with the page narrowed to the range they
are allowed to order from. A request without a buyer credential — or from a customer who is not a
wholesale buyer on this store — returns the ordinary retail page, so it never fails the request.

Each priced row carries `moq`, the smallest quantity that may be ordered, and `price_source`, which
says which arrangement set the price: `assigned` for a list assigned to that buyer, `group` for
their customer tier, `default` for the store's standard wholesale list, and `retail_fallback` when
nothing wholesale was agreed for that line.

```typescript
const buyer = new Tybrite({
  apiKey: 'tybrite_pk_live_...',
  headers: { 'x-auth-token': buyerSessionToken },
});

const { products } = await buyer.products.listProducts({
  b2b: true,
  limit: 20,
  fields: 'name,price,currency,moq,pack_size,price_source'
});
```
...
A wholesale response is specific to one buyer, so it is not cached. Ordinary retail reads keep their
caching unchanged.
...
Alongside the merchandising fields, a product carries the identifiers and measurements that
sales channels and shipping calculations need.

*   `condition` — the condition the item is sold in: `new`, `refurbished`, or `used`. Defaults
    to `new`.
*   `gtin` — the manufacturer's Global Trade Item Number (UPC, EAN, or ISBN). Sales channels
    match a listing against this to identify the product. It is distinct from the barcode a
    merchant scans in store, which may be an internal label and is never returned by this API.
*   `mpn` — the manufacturer part number, which identifies items that carry no `gtin`.
*   `weight_grams` — the shipped weight of the item, in grams.
*   `dimensions_cm` — the packed dimensions in centimetres, as free text in the merchant's own
    format. Parse it defensively rather than assuming a fixed shape.
*   `google_product_category` and `meta_product_category` — the product's category in the Google
    Shopping and Meta commerce taxonomies. Each is set separately, because the two taxonomies do
    not share category names, and both are independent of `category_name`, which is the store's
    own taxonomy.

Every one of these is optional except `condition`, so treat a `null` as "the merchant has not
supplied it" rather than as an error.

```typescript
const product = await client.products.getProduct({ id: '02525ff5-0916-4f00-857c-c2c6237e671b' });

if (product.gtin) {
  // A listing with a GTIN can be matched to the manufacturer's catalog entry.
}
```

```json Response
{
  "product_id": "02525ff5-0916-4f00-857c-c2c6237e671b",
  "name": "Sony WH-1000XM4",
  "sku": "SNY-WH1000",
  "price": 369.99,
  ...
  "meta_product_category": "electronics > audio"
}
```

`condition`, `gtin`, `mpn`, `weight_grams` and `dimensions_cm` describe an individual item, so
they belong to the variant — two sizes of one shirt carry different codes and weights.
`google_product_category`, `meta_product_category` and `tags` classify the listing as a whole and
belong to the product. All of them are returned on `listProducts` and on detail, including for a
product with several variants, and any of them can be named in `fields`.
...
Retrieve the merchant's own custom fields for a product. A merchant can define fields Galactic Core does
not — a care instruction, a country of origin, a fabric composition — and fill them per product.

Only fields the merchant published are returned. A custom field can hold a cost or an internal reference,
so a field is private unless it is explicitly marked public, and a private field is absent rather than
empty. A product with no published fields returns an empty array.
...
for (const field of custom_fields) {
  console.log(field.field_label, field.value); // "Care instructions" "Machine wash at 30"
}
```
...
The merchant controls both the field names and how many exist, so render the array rather than reading
fixed keys. See [Custom Fields](/custom-fields) for the full picture — which records carry them, what each
type looks like, and how to send values with an order. `field_type` tells you the shape of `value`: `multiselect` is an array of strings, `json`,
`dimensions` and `weight` are objects, and the rest are scalars — a `number` field arrives as a number,
not as text. `field_label` is what the merchant calls the field and is the string to show; `field_name`
is stable across label edits and is the one to key off in code.

<Note>
  `value` is `null` when the merchant defined a field but has not filled it for this product, which is
  distinct from the field not existing.
</Note>
