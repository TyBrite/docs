...
const { products, total } = await client.products.listProducts({
  search: 'Sony',
  categoryId: 'headphone-uuid',
  limit: 20,
  fields: 'name,price,stock,has_variants,product_slug' // Root-level fields
});
```
...
```typescript
const specs = await client.products.getProductSpecifications({ id: 'product-uuid' });
```
...
const { items } = await client.products.getProductCollectionItems({ id: 'flash-sale-uuid' });
```
