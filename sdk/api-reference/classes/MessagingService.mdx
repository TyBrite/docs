...
const { threads } = await client.messaging.listThreads({
  ...
  fields: 'id,subject,status,unread_count_customer' // Optional
});
```
...
const thread = await client.messaging.getThread({
  id: 'thread-uuid-123',
  fields: 'subject,priority,status' // Optional
});
```
...
const { messages } = await client.messaging.getThreadMessages({
  id: 'thread-uuid-123',
  fields: 'message_content,sender_type,created_at' // Optional
});
...
await client.messaging.sendMessage({
  id: 'thread-uuid-123',
  requestBody: {
    message: "Thank you for the update. I'll check my account."
  }
});
```
...
<Tip>
  **Pro UX Tip:** Use the `unread_count_customer` field from `listThreads` to display a badge on your "Support" icon to notify customers of agent replies.
</Tip>
