...
<Note>
  This is a WebSocket, not a request that counts toward your monthly request quota. To open one yourself instead of using the helper, connect to `GET /v1/messaging/threads/{id}/subscribe` with `?api_key=` and either `?auth_token=` or `?external_auth=` in the URL; the server pushes one JSON frame per new message: `{ "type": "message.created", "payload": { "thread_id", "message" } }`.
</Note>
