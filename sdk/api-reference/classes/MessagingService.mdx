...
// returns e.g. 3   (each thread carries its own unread_count_customer)
```
...
// returns { thread: { id, status: 'resolved', priority: 'normal', is_archived: true, is_muted: true, is_pinned: false, … } }
```
...
// returns { success: true, thread_id: '06084af6-7334-43d6-82c0-935f92335a25' }
```
...
// returns { message: { id: '33e2c744-37a9-42df-a9df-447715526a41', created_at: '2026-06-20T11:29:02.467216+00:00' } }
```
...
// returns { success: true, message_id: 'cc64ce9a-51ba-4860-ba1a-85f14899f5bf', deleted_at: '2026-06-21T10:16:33.419Z' }
```
...
`thread_type` is inferred when it is not supplied: `order_inquiry` when `order_id` is present,
...
A thread's new messages arrive as they are sent over a **WebSocket**, rather than on the interval a `getThreadMessages` poll would impose. The `subscribeToThread` helper is exported from the SDK package root and uses the runtime's built-in WebSocket, so it runs on browsers, Deno, Bun, and Node 22+ with no additional dependency.
