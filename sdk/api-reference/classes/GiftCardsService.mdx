...
<Note>
  `customerId` is **strictly required** for this endpoint to identify which wallet to retrieve. In addition, an `xAuthToken` (the customer's session token) must be supplied so Galactic Core can verify the requester owns that wallet.
</Note>
...
<Note>
  When listing gift cards for a customer, the resolved `xAuthToken` session must match the `customerId` parameter. Otherwise the request returns 403. This means a leaked publishable key alone cannot enumerate another customer's gift cards — the customer's session token is required.
</Note>
...
The customer's session token must match the requested `customerId` before any wallet contents are returned.
