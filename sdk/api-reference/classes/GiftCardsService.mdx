...
const order = await client.orders.createOrder({
  ...
  requestBody: {
    // ...other order details...
    gift_cards: [
      { code: 'TYB-GIFT-123-456', amount: 5000 }
    ]
  }
});
```
...
<Tip>
  To let customers pay for partial orders with gift cards, use `checkGiftCard` to see the available balance first, then include it in the `gift_cards` array when calling `createOrder`.
</Tip>
