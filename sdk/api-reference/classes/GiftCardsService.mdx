...
// returns { order: { id, total_amount, payment_status, … }, post_processing_warnings? }
...
// returns { user: { id, email, … }, session: { access_token, refresh_token, expires_at } }
...
// returns { gift_cards: [{ code, balance, status, expiry_date }, …] }
```
