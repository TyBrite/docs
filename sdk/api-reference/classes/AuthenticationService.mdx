...
The email must be a real, deliverable address. Disposable and placeholder domains (`example.com`, `test.com`, known throwaway-mail providers, and similar) are rejected with `400 invalid_request`, so a sign-up form that submits a dummy address gets a validation error rather than a dead account.
...
// returns same shape as login: { message, user, customer, session }
...
// returns { message: 'Password reset email sent', email: 'jane.doe@tybritelabs.com' }
```
...
//     user: { id: 'df648f68-39b3-4ba6-9be2-ff0a9393c02b', email: 'jane.doe@tybritelabs.com' }
//   }
```
...
// returns { message: 'Token refreshed successfully', session: {
//       access_token: 'eyJ...<redacted>.signature',
...
// returns { items: [ { product_name, variant_name, quantity, unit_price, total_price, ... } ] }
...
// returns { data: { id, order_number, payment_status: 'paid', total, ... } }
...
// returns { message: 'Token refreshed successfully', session: { access_token, refresh_token, ... } }
