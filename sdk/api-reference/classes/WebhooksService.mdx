...
Available test event types: `order.created`, `order.paid`, `order.fulfilled`, `order.cancelled`, `payment.succeeded`, `payment.failed`, `customer.created`, `product.created`, `product.stock_low`, `cart.abandoned`, `gift_card.issued`, `promotion.applied`, `feed.sync.completed`, `channel.sync.completed`.
