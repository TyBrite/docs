...
<Tip>
  Signature verification runs against the raw request body, before JSON parsing. A parser may reformat the JSON into a different byte sequence, which no longer matches the signature.
</Tip>
...
// returns { webhook_endpoint: { id, url, events: ['*'], enabled: true, signing_secret, … } }
```
...
| Cart & shopping intent | `cart.created`, `cart.updated`, `cart.abandoned`, `checkout.abandoned`, `wishlist.added` |
...
| Pricing | `pricing_rule.created`, `pricing_rule.activated`, `pricing_rule.updated`, `pricing_rule.deactivated` |
| Content & collections | `collection.created`, `collection.updated` (includes membership changes, carrying `members_changed`), `post.published`, `lookbook.published`, `review.approved` |
| Deletion & restore | `product.deleted`, `product_variant.deleted`, `promotion.deleted`, `collection.deleted`, `post.deleted`, `lookbook.deleted`, `pricing_rule.deleted`, `subcategory.deleted`, `customer.deleted` — the record has left the API. Each has a matching `.restored` (deleting is reversible for 90 days); payloads carry `deleted_at` and `restorable_until`. Deleting a product also deletes its variants |
| Visibility | `category.activated` / `category.deactivated`, `subcategory.activated` / `subcategory.deactivated` — a category switched off is filtered out of every catalog read. Categories cannot be deleted, so these are their only lifecycle events |
...
| Wholesale credit &amp; approvals | `b2b.credit.threshold_reached`, `b2b.credit.exhausted`, `b2b.buyer.on_hold`, `b2b.buyer.released`, `b2b.approval.requested` / `.approved` / `.rejected` — each fires on the crossing, not the level |
| Wholesale replenishment | `b2b.replenishment.needed` — fires in the buying store about its own stock, carrying the supplier that line was last bought from |
...
// returns { webhook_endpoint: { …, enabled: true, consecutive_failures: 0, disabled_reason: null, disabled_at: null } }
```
...
// returns { webhook_endpoint: { …, enabled: false } }
...
// returns { success: true, message: 'Webhook endpoint deleted' }
```
...
Available test event types: `order.created`, `order.paid`, `order.fulfilled`, `order.shipped`, `order.cancelled`, `payment.succeeded`, `payment.failed`, `customer.created`, `product.created`, `product.stock_low`, `cart.abandoned`, `checkout.abandoned`, `wishlist.added`, `gift_card.issued`, `promotion.applied`, `promotion.activated`, `pricing_rule.activated`, `collection.created`, `post.published`, `review.approved`, `store.updated`, `payment_provider.connected`, `feature.status_changed`, `feed.sync.completed`, `channel.sync.completed`.
