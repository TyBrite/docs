...
the highest view→purchase conversion over a time window. They back storefront surfaces such as a
"Popular now" homepage shelf, a "Trending in carts" section, or a product-page "hot" badge.

All three reads are publishable-key accessible, so they are safe to call from a browser or app, and
they are available on every plan. Each returns product ids and a `score`; the product details come
from [Products](/sdk/api-reference/classes/ProductsService).
...
## When there is not enough activity yet

A store with little traffic in the window would return an empty or one-item shelf, so the list is
topped up from featured and recently-added products and the response carries `backfilled: true`.

Those entries are real, purchasable products and are safe to display. What changes is what `score`
means: for a padded entry it is a ranking weight, not a view count or a conversion ratio. A new
store, or a short window on a quiet one, will see `backfilled: true` routinely — it is the normal
state early on, not an error.

```typescript
const popular = await client.discovery.getMostViewedProducts({ windowHours: 24, limit: 8 });

if (popular.backfilled) {
  // Not enough views in the window to rank by. The products are still worth showing —
  // label the shelf generically rather than calling it "Most viewed".
  renderShelf('Popular picks', popular.products);
} else {
  renderShelf('Most viewed this week', popular.products);
}
```
...
`score` is the view count in the window (unless `backfilled` is `true` — see above). A product's presence in `getMostViewedProducts({ windowHours: 1 })` is a natural "hot right now" signal for a PDP badge.
...
// returns { metric: 'most-added-to-cart', window_hours: 168,
...
// returns { metric: 'best-converting', window_hours: 720,
//     products: [
