...
## How wholesale works in Tybrite

A wholesale order moves through a fixed sequence, and each step maps to one or two methods here.
...
- `xTimestamp` / `xSignature` — required. A direct order must be signed, like a standard order: `xTimestamp` is the current Unix time in milliseconds, and `xSignature` is the Base64 HMAC-SHA256 of `` `${xTimestamp}.${rawJsonBody}` `` using the store's signing secret. See [Signing requests](/authentication#advanced-security-hmac-signing).
