...
<Note>
  Discounts are calculated server-side: `calculatePromotionDiscount` evaluates one promotion
  against a cart, and `calculateBestPromotion` selects the highest-value one. Both return the
  same figure the merchant sees in their POS, which a storefront reimplementation of
  `subtotal × percentage`, bundle math, or BOGO logic would have to keep matching as those
  rules change. See [Calculating a discount](#calculating-a-discount) below.
</Note>
...
// returns promo.bundle_products = [{ productId, quantity, discountedPrice }]
...
### Promotions limited to a market

A promotion carries `target_countries` and `target_regions`. When both are empty the promotion applies to every shopper, which is what a promotion with no targeting set carries. When either holds values, the shopper's location must match for the promotion to be eligible.

`calculatePromotionDiscount` and `calculateBestPromotion` accept the shopper's `country` (ISO 3166-1 alpha-2) and `region` alongside the cart:

```typescript
const result = await client.promotions.calculatePromotionDiscount({
  id: 'promotion_uuid',
  requestBody: {
    cart: [{ product_id: 'prod_a', quantity: 2, price: 25.00 }],
    country: 'GB',
    region: 'Scotland',
  },
});
...
Both fields are optional. When you omit them, Galactic Core reads the shopper's country from the network edge, so a browser calling directly is located without you sending anything — send them explicitly when your storefront renders on a server, or when you are previewing a market other than the caller's own.

A promotion the shopper's location excludes returns `eligible: false` with a `reason` naming the restriction, and `calculateBestPromotion` passes over it entirely rather than returning a deal that would be refused at checkout. The same evaluation runs when the order is placed, so a targeted promotion cannot be applied by sending a different country than the one the order ships from.

Targeting composes with every other condition: a promotion may be limited to a market *and* carry a minimum purchase, a date window, and time-of-day restrictions, and all of them must pass.
...
// returns same Promotion shape as above, scoped to merchant_store_id
...
// returns { promotion_id, type, eligible, discount, reason }
...
// returns { promotion: { id, name, type } | null, discount }
```
...
| `target_countries` | `array` | ISO 3166-1 alpha-2 codes the promotion is limited to. An empty array means it applies everywhere. |
| `target_regions` | `array` | Region or state names the promotion is limited to, matched case-insensitively. An empty array means every region. |
