...
Scopes are **enforced on every request** — a key is rejected with `403 insufficient_scope` if it calls an endpoint it wasn't granted, so request exactly what you need and nothing more.
...
- **The merchant disconnects you** from their store's **Integrations → Developer → Connected Apps** panel. Galactic Core deactivates the key pair automatically — you don't need to do anything.
