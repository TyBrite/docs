...
- **Sign the raw header string and the raw body bytes.** Re-serialising the parsed JSON changes key
  order and spacing, and the signature will not match.
- **`X-Timestamp` is milliseconds**, so compare it against `Date.now()` directly.
- **Require the signature.** Galactic Core sends no `X-Signature` at all when an extension has no
  signing secret configured, so an endpoint that verifies "only if the header is present" is open to
  anyone who omits it.
...
Most agencies never need this interface. Stripe, Lemon Squeezy and Paddle are supported directly, and
Paystack in its markets, so an agency picks one in their portal and connects it. Build a
`SubscriptionBillingProvider` when you need a processor we don't ship — a regional gateway, or your
own billing system that already holds the customer relationship.
...
These are ceilings, not suggestions. A `timeout_ms` on your extension can only make its budget
*smaller* — it can never raise the ceiling, so an endpoint written for a longer budget is cut off at
the limit for its class.
...
The shopper-facing budgets are tight on purpose: someone is waiting on a page render. Admin-plane work
gets more room because a merchant clicked a button and can wait — a real billing provider needs two
round trips (authenticate, then create) and does not fit a checkout budget.

<Warning>
  Measure your endpoint's **slowest** path against these, not its typical one. A lookup for a reference
  the provider has never seen is often far slower than a normal call, and an endpoint that sits just
  under the ceiling on average will fail intermittently — which looks like flakiness rather than
  misconfiguration.
</Warning>
