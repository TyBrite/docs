...
| `SubscriptionBillingProvider` | `{ operation: 'createSubscription', store_id, plan, period, amount_cents, currency, subscriber_email, return_url, cancel_url }` / `{ operation: 'getSubscription' \| 'cancelSubscription', subscription_id }` | `createSubscription → { subscription_id, status, redirect_url?, provider }`; `getSubscription → { subscription_id, status, next_billing_at?, provider }`; `cancelSubscription → { subscription_id, status, provider }` |
