...
Isolation and permission are separate questions, and both are answered in the database. Beyond *which
store* a row belongs to, each staff member's role determines *which tables they may read and write* —
...
processor, letting an automation run unattended forever. Those are covered in
