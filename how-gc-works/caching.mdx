...
Business data — the catalog, pricing rules, currencies — is cached without a TTL. Rather than expiring on
a timer, it is kept correct by invalidation: when a merchant edits a product, changes a price, or adjusts
stock, the write path refreshes the affected entries at once. The cache is therefore always current,
without the window of staleness a timed cache carries while it waits out its TTL.
...
Short-lived external data, such as geolocation lookups and currency snapshots, does use a TTL, since it is
not something a merchant writes. The business catalog, though, never depends on expiry for correctness.
