...
Isolation comes at two levels, and which one applies depends on the tenant rather than on the code.
...
<Columns cols={2}>
  ...
  <Card title="An incident reaches one tenant" icon="shield-halved">
...
The same codebase serves both tiers, so a tenant can start on the shared platform and move to a dedicated
deployment without a rewrite. Between them the two tiers cover the confidentiality and integrity sides of
the CIA triad: row-level security and, where it applies, physical isolation keep each tenant's data
confidential, and the server-side pricing authority and ACID writes described in the
