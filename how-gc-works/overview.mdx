...
customer metrics in one transaction. And every tenant is isolated — by row-level security on the shared
platform, and on a dedicated stack of their own where a tenant needs one.
