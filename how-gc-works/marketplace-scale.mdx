...
<Note>
  **A platform operated by an agency spans both shapes.** Its merchants sell under their own
  storefronts, catalogues and stock, and in that mode they do not contend with each other the way
  sellers on one marketplace storefront do — for them, this page describes a problem they do not
  have. See [Growing the platform](/agencies#growing-the-platform). Where an agency also runs a
  marketplace, the merchants taking part in it are sellers on one storefront like any others, and
  everything below applies to that storefront.
</Note>
...
<Columns cols={2}>
  <Card title="Between operators" icon="server">
    A marketplace can run on its own stack — its own database, cache, compute and object storage —
    and at the volumes this page describes it does. Nothing is then shared with another operator, so
    one marketplace's peak is invisible to every other marketplace, and hardware is provisioned for
    the operator's expected traffic rather than drawn from a shared tier. A marketplace sharing a
    deployment with others is isolated from them by the same store boundary that separates any two
    stores; what it does not get is a capacity ceiling of its own.
  </Card>
...
<Columns cols={2}>
  ...
  <Card title="Order writes are queued under load" icon="layer-group">
    On a marketplace deployment, order creation is deferred once a store's pending-order depth crosses a
    threshold: a surge from one seller becomes latency for that seller rather than errors for the
    marketplace. Below the threshold, and on deployments where deferral is off, orders are created
    synchronously. When a create is deferred it returns `202 Accepted` with the `Idempotency-Key` the
    order will be created under, instead of the synchronous `201` — a client fetches the order by
    replaying the create with that key. Deferral is automatic once enabled, and enabled by default for
    marketplace deployments; other deployments opt in, because returning `202` changes what a storefront
    must handle and only an integration that polls by idempotency key should receive it. Queue depth is
    visible to the operator as it happens.
  </Card>
