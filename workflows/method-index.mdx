...
Where the use-case pages cover the main flows, this one covers the whole surface: every method on
every service, grouped by the task it does, each with a short call and its response shape. A method
absent from the other pages appears here. Setup is the two-client pattern from
[the overview](/workflows/introduction) — `storefront` on a publishable key, `server` on a secret
key.
...
Every method on every service appears either in a use-case page or here. The full parameter list and
field-by-field response of any method live on its
