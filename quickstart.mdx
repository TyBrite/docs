---
title: "Quickstart"
description: "Get a key, initialize the SDK or a REST client, and make your first authenticated call against a live store."
---

Four steps take you from an empty project to a rendered product list: get a key, initialize a
client, read the catalog, and read the store's own configuration.
...
<AccordionGroup>
  <Accordion icon="key" title="GC Admin">
    Login to the [Galactic Core Dashboard](https://gc.tybritelabs.com/dashboard) and navigate to **Integrations → API Keys**. 
  ...
  <Accordion icon="shield" title="Where each key belongs">
    A secret key belongs only in server-side code. Publishable keys cover search and product
    browsing; checkout and accounting run from your server with a secret key.
  </Accordion>
</AccordionGroup>
...
| `marketplace` | `mode` tells you whether the key you called with reads a single store (`standard`) or a multi-merchant marketplace (`marketplace`). |
