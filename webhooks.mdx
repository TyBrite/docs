...
Deleting is reversible for 90 days, so each of these has a matching `.restored` event. `restorable_until` is there so you can choose: tear your copy down immediately and rebuild it if the restore arrives, or hold it until the window closes.
