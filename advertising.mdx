...
<Note>
  ...
  Ads is available today; Meta and Microsoft are planned on the same interface.
</Note>
...
Advertising is a Premium feature, connected under **Integrations → Google Ads** in the admin. Some
capabilities depend on the ad platform approving the account for API access; until then the connection
reports its status clearly rather than failing silently.
