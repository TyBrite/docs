...
<CardGroup cols={2}>
  <Card title="AI campaign builder" icon="wand-magic-sparkles">
    ...
    review before anything goes live. It checks the ad platform's current guidance before writing,
    which is on by default so a campaign is not built from stale advice.
  </Card>
  <Card title="Experiments" icon="flask">
    Keep the versions the builder wrote and run them against each other. A winner is only declared
    once enough people have seen every version **and** the difference is large enough to be real —
    neither condition alone is enough. Promoting a winner pauses the losing ads, so it respects the
    same approval threshold and spending limits as any other change that costs money.
  </Card>
  <Card title="Ask the assistant" icon="comments">
    The in-app assistant reaches the same builder, and can start from a question rather than a brief —
    what is worth advertising given what is actually selling, whether it is worth spending at all, or
    what limits to set before letting anything run unattended. Everything it produces is a draft.
  </Card>
